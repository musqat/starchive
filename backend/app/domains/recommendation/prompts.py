...
LIKED_LIMIT = 15
REASON_CHARS = 60
CITE_LIMIT = 3  # 열 개가 다 같은 제목을 들면 같은 카드를 돌려쓰는 것처럼 읽힌다
...
좋아한 작품의 제목을 드는 이유는 {CITE_LIMIT}개까지만. 같은 제목을 두 번 들지 않는다.
나머지는 그 작품 자체의 무엇이 볼 만한지 쓴다. 연결이 약하면 억지로 갖다 붙이지 않는다.
