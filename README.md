...
앱 화면은 [mobile/README.md](mobile/README.md) 에 있다.
...
기록이 적으면 이웃 판정에 걸리는 사람이 많아 결과가 인기순으로 수렴한다. 인기 스릴러만
...
앱은 쿠키를 쓰지 않는다. `/auth/token` 으로 토큰을 본문에 받아 `Authorization: Bearer` 헤더로
보낸다. 로그인 잠금과 검색 호출 제한은 두 경로에 똑같이 걸린다.
...
DB 가 필요한 테스트는 `db` 표시로 갈라 두고, CI 는 컨테이너 없이 도는 43 개만 돌린다.
